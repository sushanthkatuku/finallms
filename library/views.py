...
#con=mysql.connector.connect(user='root',password='',database='lms')
#c=con.cursor()
...
def deleteBook(request):
    ...
    models.StudentExtra.objects.filter(id=bid).delete()
    students=models.StudentExtra.objects.all()
    return render(request,'library/viewstudent.html',{'students':students})
...
def updateBook(request):
    ...
    s="update library_book set name='"+name+"',author='"+author+"',category='"+category+"' where id="+bid
    print("SQl is ",s)
    c.execute(s)
    con.commit()
...
def editBook(request):
    bid=request.GET.get('bid')
    c.execute("select * from library_book where id="+bid)
    bdata=c.fetchall()
    print("Book id ",bdata)
    #bdata=models.Book.objects.filter(id=bid)
    bdata1={'id':bdata[0][0],'name':bdata[0][1],'author':bdata[0][3],'category':bdata[0][4]}
    return render(request,'library/editBook.html',{'book':bdata1})
